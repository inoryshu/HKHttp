...
static HKLRUManager *manager = nil;

static NSMutableArray *operationQueue = nil;

static NSString *const RYLRUManagerName = @"RYLRUManagerName";

@implementation HKLRUManager
+ (HKLRUManager *)shareManager {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[HKLRUManager alloc] init];
        if ([[NSUserDefaults standardUserDefaults] objectForKey:RYLRUManagerName]) {
            operationQueue = [NSMutableArray arrayWithArray:(NSArray *)[[NSUserDefaults standardUserDefaults] objectForKey:RYLRUManagerName]];
        }else {
...
- (void)addFileNode:(NSString *)filename {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:[operationQueue copy] forKey:RYLRUManagerName];
